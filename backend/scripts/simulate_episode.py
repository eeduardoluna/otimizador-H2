...
def simulate(use_rl: bool = True, seed: int = 42, verbose: bool = True) -> dict:
    # Força o mesmo seed numpy para garantir clima idêntico
    np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    if args.compare:
        ...
        pct = diff / abs(result_bl["total_profit"]) * 100 if result_bl["total_profit"] != 0 else 0
        print(f"\n💰 Diferença de lucro: R$ {diff:+.2f} ({pct:+.1f}%)")
    else:
